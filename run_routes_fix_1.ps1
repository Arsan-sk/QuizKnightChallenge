...
export function StudentReportTable({ data, questions = [], quizId }: StudentReportTableProps) {
 ...
 = '(?s)\Qconst userAns = answersArray[idx];\E.*?\Q// Accumulate approximate time for this question\E'
 = @'
            ...
            // Count correct when the user's answer equals the stored correctAnswer (robust matching)
            if (userAns !== undefined && userAns !== null && String(userAns).trim().toLowerCase() === String(q.correctAnswer).trim().toLowerCase()) {
              qd.correctCount++;
            }
