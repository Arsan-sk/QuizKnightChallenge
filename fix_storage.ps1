...
export function StudentReportTable({ data, questions = [], quizId }: StudentReportTableProps) {
 ...
 = Get-Content -Path server/storage.ts -Raw

 = '(?s)\Q        // Get global rank\E\n\s*\Q        const leaderboard = await this.getGlobalLeaderboard(0); let winStreak = 0; for (const r of results) { if (r.score >= 70) { winStreak++; } else { break; } }\E\n\s*\Q        const rank = leaderboard.findIndex(entry => entry.id === userId) + 1;\E'
 = @'
        // Get global rank
        ...
        let winStreak = 0;
        let currentStreak = 0;
        // Sort results chronologically (oldest first) to accurately find max consecutive distinctions (>= 90)
...
, , )
 ...
 = '(?s)\Q          .groupBy(users.id, users.username, users.name, users.profilePicture, users.role, users.points)\E\n\s*\Q          .orderBy(desc(sqlSUM()), desc(users.points))\E\n\s*\Q          .limit(limit);\E'
 = @'
          .groupBy(users.id, users.username, users.name, users.profilePicture, users.role, users.points)
          .orderBy(desc(sqlCOALESCE(SUM(), 0)), desc(users.points));

        if (limit > 0) {
          return await leaderboard.limit(limit);
        }
'@
import { StudentReport } from "@/types/analytics";
import { formatTime } from "@/utils/analytics";
...
Set-Content -Path server/storage.ts -Value import { StudentReport } from "@/types/analytics";
import { formatTime } from "@/utils/analytics";
import { useState } from "react";
import { ChevronDown, Filter, X, Check, XCircle } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
