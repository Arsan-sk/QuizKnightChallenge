...
export function StudentReportTable({ data, questions = [], quizId }: StudentReportTableProps) {
 ...
 = '(?s)\Qconst result = await storage.createResult({\E.*?\Q          answers: JSON.stringify(userAnswers || []),\E\n\s*\}\);'
 = @'
        const result = await storage.createResult({
          ...
          tabSwitchCount: validatedData.tabSwitchCount || 0,
          copyPasteAttempts: validatedData.copyPasteAttempts || 0,
          proctoringFlags: validatedData.proctoringFlags || 0,
        });
'@
import { StudentReport } from "@/types/analytics";
import { formatTime } from "@/utils/analytics";
