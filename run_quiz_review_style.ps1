...
export function StudentReportTable({ data, questions = [], quizId }: StudentReportTableProps) {
 ...
 = Get-Content -Path client/src/pages/quiz-review.tsx -Raw
 = '(?s)\Q            {questions.map((question, index) => {\E.*?\Q              return (\E(.*?)\Q              );\E\n\s*\Q            })}\E'
 = @'
            {questions.map((question, index) => {
              const studentAnswer = userAnswers[index] || "";
              const isRight = studentAnswer === question.correctAnswer;
              const hasAttempted = studentAnswer !== undefined && studentAnswer !== null && studentAnswer !== "";

              return (
                 <div key={question.id || index} className="bg-[#131316] rounded-2xl p-6 border border-white/5">
                    <div className="flex items-start justify-between mb-4 gap-4">
                       <h4 className="text-lg font-bold text-white"><span className="text-zinc-600 mr-2">{index + 1}.</span>{question.questionText}</h4>
                       {hasAttempted ? (
                          isRight ? (
                             <div className="flex items-center gap-2">
                                <span className="bg-emerald-500/10 text-emerald-400 font-bold px-3 py-1 rounded-full text-[10px] tracking-widest uppercase">
                                  +{question.points ?? 1} PTS
                                </span>
                                <div className="w-8 h-8 shrink-0 rounded-full bg-emerald-500/20 border border-emerald-500/40 flex items-center justify-center">
                                   <CheckCircle className="w-4 h-4 text-emerald-400" />
                                </div>
                             </div>
                          ) : (
                             <div className="flex items-center gap-2">
                                <span className="bg-rose-500/10 text-rose-400 font-bold px-3 py-1 rounded-full text-[10px] tracking-widest uppercase">
                                  0 PTS
                                </span>
                                <div className="w-8 h-8 shrink-0 rounded-full bg-rose-500/20 border border-rose-500/40 flex items-center justify-center">
                                   <XCircle className="w-4 h-4 text-rose-400" />
                                </div>
                             </div>
                          )
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3 mt-4">
                       {Array.isArray(question.options) ? question.options.map((opt: string, oIdx: number) => {
                          const isCheckedByStudent = opt === studentAnswer;
                          ...
                          return (
                             <div key={oIdx} className={p-4 rounded-xl flex items-center justify-between text-sm font-medium }>
                                <span>{opt}</span>
                                {isCheckedByStudent && <div className="w-2 h-2 rounded-full bg-current" />}
                             </div>
                          );
                    ...
                    {(!isRight) && hasAttempted && question.explanation && (
                       <div className="mt-4 p-4 rounded-xl bg-indigo-500/10 border border-indigo-500/20 text-indigo-200 text-sm">
                          <span className="font-bold mr-2 uppercase text-[10px] tracking-widest text-indigo-400">Explanation:</span>
...
, , )
Set-Content -Path client/src/pages/quiz-review.tsx -Value import { StudentReport } from "@/types/analytics";
import { formatTime } from "@/utils/analytics";
import { useState } from "react";
import { ChevronDown, Filter, X, Check, XCircle } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
