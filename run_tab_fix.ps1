...
export function StudentReportTable({ data, questions = [], quizId }: StudentReportTableProps) {
 ...
 = Get-Content -Path client/src/components/analytics/StudentReportTable.tsx -Raw
 = '(?s)\Q{/* Modal Content - Tabs */}\E\s*<div className="bg-\[#1c1c21\] pt-4 px-4 md:px-8 border-b border-white/5 flex gap-8 text-xs font-bold uppercase tracking-widest overflow-x-auto">\s*<button'
 = @'
              {/* Modal Content - Tabs */}
              <div className="shrink-0 bg-[#1c1c21] pt-4 px-4 md:px-8 border-b border-white/5 flex gap-8 text-xs font-bold uppercase tracking-widest overflow-x-auto relative z-20" style={{ minHeight: '60px' }}>
                 <button
'@
import { StudentReport } from "@/types/analytics";
import { formatTime } from "@/utils/analytics";
...
, , )
Set-Content -Path client/src/components/analytics/StudentReportTable.tsx -Value import { StudentReport } from "@/types/analytics";
import { formatTime } from "@/utils/analytics";
import { useState } from "react";
import { ChevronDown, Filter, X, Check, XCircle } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
