...
export function StudentReportTable({ data, questions = [], quizId }: StudentReportTableProps) {
 ...
 = Get-Content -Path client/src/components/analytics/QuestionChart.tsx -Raw

 = '(?s)\Qimport { motion } from "framer-motion";\E'
 = @'
...
export function QuestionChart({ data }: QuestionChartProps) {
  ...
  if (!isExpanded) {
    toughestQuestions = toughestQuestions.slice(0, 4);
  }
'@
import { StudentReport } from "@/types/analytics";
import { formatTime } from "@/utils/analytics";
...
, , )
 ...
 = '(?s)\Q      <div className="space-y-6 flex-1">\E.*?\Q      <button className="w-full mt-8 bg-white/5 hover:bg-white/10 text-white text-xs font-bold uppercase tracking-widest py-4 rounded-xl transition-colors border border-white/5 shadow-sm">\E.*?\Q      </button>\E'
 = @'
      <div className={space-y-6 flex-1 } style={isExpanded ? { maxHeight: '300px' } : {}}>
            ...
            return (
              <div key={q.questionId} className="w-full relative group">        
                <div className="flex justify-between items-center mb-3">        
                  <span className="text-xs font-bold text-zinc-300 truncate pr-4 max-w-[200px]" title={q.questionText}>
                    Q{i + 1}. {q.questionText}
                  </span>
                  <span className={	ext-[11px] font-bold  shrink-0}>
                ...
                <div className="w-full h-1.5 bg-[#131316] rounded-full overflow-hidden border border-white/5">
                  <motion.div
                    ...
                    transition={{ duration: 1, delay: (!isExpanded ? i : 0) * 0.1, ease: "easeOut" }}
      ...
      {data.length > 4 && (
        <button 
          ...
          className="w-full mt-8 bg-white/5 hover:bg-white/10 text-white text-xs font-bold uppercase tracking-widest py-4 rounded-xl transition-colors border border-white/5 shadow-sm shrink-0"
        >
          {isExpanded ? "Collapse View" : Review All  Questions}
        </button>
      )}
'@
import { StudentReport } from "@/types/analytics";
...
Set-Content -Path client/src/components/analytics/QuestionChart.tsx -Value import { StudentReport } from "@/types/analytics";
import { formatTime } from "@/utils/analytics";
import { useState } from "react";
import { ChevronDown, Filter, X, Check, XCircle } from "lucide-react";
import { motion, AnimatePresence } from "framer-motion";
